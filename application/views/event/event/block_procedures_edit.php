...
<?php foreach ($procedures_d as $done):?>
	<tr style="background-color: <?php echo ($done['price_brut'] == 0) ? "#fff0ed" : "#edfff8" ?>;">
		<td><?php echo $done['volume']; ?> x <?php echo $done['procedures']['name']; ?></td>
		...
		<td>
			<form action="<?php echo base_url(); ?>events/edit_price/<?php echo $event_info['id']; ?>" id="form<?php echo $done['id']; ?>" method="post" autocomplete="off" class="form-inline">
				...
				<input type="hidden" name="ori_net_price" value="<?php echo ($done['price_ori_net'] != 0) ? $done['price_ori_net'] : $done['price_net']; ?>"/>
		...
		<td>
		<?php
			if ($done['price_ori_net'] != 0)
			{
				
				$change = round((($done['net_price']-$done['price_ori_net'])/$done['price_ori_net'])*100);
				echo ($change < 0) ? 
